...
| 记住上下文 | 每次都要手动拼接历史对话 | Memory 自动管理 |
...
Chain 是**把多个步骤组合成一个整体**的概念。在 LCEL 出现之前，LangChain 有很多内置 Chain 类（如 `LLMChain`、`ConversationChain`），现在官方推荐用 LCEL 自己拼，更灵活。
...
模型本身是**无状态**的——每次调用都是独立的，它不记得上一次说了什么。Memory 组件用来**保存和管理对话历史**，让模型「记住」上下文。
...
| Memory 类型 | 存储方式 | 特点 |
|------------|---------|------|
| `ConversationBufferMemory` | 保存全部历史 | 简单直接，但 token 会越来越多 |
| `ConversationBufferWindowMemory` | 只保留最近 N 轮 | 防止 token 爆炸 |
| `ConversationSummaryMemory` | 用模型总结历史 | 压缩信息，适合长对话 |
| `ConversationSummaryBufferMemory` | 总结 + 缓冲 | 达到长度就总结，兼顾两者 |

### 代码示例

#### 1. ConversationBufferMemory（保存全部）
...
print(conversation.invoke("我叫小明，今年25岁")["response"])
print(conversation.invoke("我叫什么名字？")["response"])
# AI 能记住：你叫小明
```

#### 2. ConversationBufferWindowMemory（只留最近几轮）
...
conversation.invoke("我说了A")     # 第1轮
conversation.invoke("我说了B")     # 第2轮
conversation.invoke("我说了C")     # 第3轮，此时第1轮已被遗忘

# 问第1轮的内容 → 模型不记得了
conversation.invoke("我第一个说的是什么？")  # 忘了
```

#### 3. 手动管理消息（更灵活）

实际上，最灵活的方式是**自己管理消息列表**，不依赖 Memory 类：
...
print(chat("我叫小明"))
print(chat("我叫什么？"))  # 记得：小明
```

> **建议**：简单场景用 Memory 类，需要精细控制时手动管理消息列表。
...
LangChain 新版推荐使用 `create_tool_calling_agent` 或 `create_react_agent`：

#### 方式一：create_tool_calling_agent（推荐）
...
# 3. 创建提示词（必须包含 agent_scratchpad）
prompt = ChatPromptTemplate.from_messages([
    ("system", "你是一个数学助手，可以使用提供的工具来计算"),
    ("human", "{input}"),
...
# 5. 创建执行器（AgentExecutor 负责循环执行）
agent_executor = AgentExecutor(agent=agent, tools=tools, verbose=True)

# 6. 运行
result = agent_executor.invoke({"input": "3 的 4 次方是多少？然后再乘以 2"})
print(result["output"])
```

> `verbose=True` 会打印 Agent 的完整思考过程，调试时非常有用。

#### Agent 执行过程（verbose 输出示例）

```
> Entering new AgentExecutor chain...

Agent: 我需要先计算 3 的 4 次方
...
Agent: 计算完成
Final Answer: 3 的 4 次方是 81，再乘以 2 等于 162。
```
...
agent_executor = AgentExecutor(
    ...
    early_stopping_method="generate",  # 超限时让模型总结
)
```
...
# 现在能记住之前的对话了
agent_executor.invoke({"input": "3 + 5 等于多少？"})
agent_executor.invoke({"input": "再乘以 3 呢？"})  # 记得上面是 8
```
...
| Agent 类型 | 说明 | 适用场景 |
|-----------|------|---------|
| `create_tool_calling_agent` | 用模型原生 tool calling | OpenAI 等支持工具调用的模型 |
| `create_react_agent` | 经典 ReAct 推理 | 通用，文本推理 |
| `create_structured_chat_agent` | 支持复杂参数结构 | 工具参数复杂时 |
| `create_json_agent` | 专门操作 JSON 数据 | JSON 数据交互 |

> **建议**：如果你的模型支持原生 tool calling（如 GPT-4），优先用 `create_tool_calling_agent`，效果最好、最稳定。
...
| 特性 | Chain（LCEL） | Agent（AgentExecutor） | LangGraph |
|------|--------------|----------------------|-----------|
...
LangGraph 提供了开箱即用的预构建图：

```python
from langgraph.prebuilt import create_react_agent

# 一行创建 ReAct Agent（内部已用图实现）
agent = create_react_agent(model, tools)

result = agent.invoke({"messages": [HumanMessage("北京今天天气？")]})
```

> `create_react_agent` 是 LangGraph 对传统 Agent 的重新实现，比 `AgentExecutor` 更可控、更透明。官方推荐新项目用 LangGraph 版本。
...
# Agent 会自己决定用哪个工具
print(executor.invoke({"input": "现在是几点？"})["output"])
print(executor.invoke({"input": "帮我算一下 (15 + 27) * 3"})["output"])
print(executor.invoke({"input": "把 'Hello World' 反转"})["output"])
```
...
```python
# 用 BufferWindowMemory 而非 BufferMemory（限制历史长度）
memory = ConversationBufferWindowMemory(k=5)

# 用 ConversationSummaryMemory 压缩长对话
...
```python
# Agent 一定要设置 max_iterations 和错误处理
agent_executor = AgentExecutor(
    agent=agent,
    tools=tools,
...
| **Tool** | 让模型执行实际操作 | 手和脚 |
| **Agent** | 自主决策的智能体 | 独立思考的助手 |
| **Callback** | 执行过程中的钩子 | 监工 |
| **LangGraph** | 用图构建有状态、可循环的工作流 | 流程图引擎 |
...
```
入门:  Model → Prompt → Output Parser → LCEL
       （掌握基础组件和管道语法）

进阶:  Chain → Memory → Retriever → RAG
       （构建实用应用：对话机器人、文档问答）

高级:  Tool → Agent → Callback
       （自主智能体、可观测性、生产级应用）
